...
module game {
  ...
  export var shouldRotateBoard: boolean = false;
  ...
  export function cellClickedOne(row: number, col: number): void {
    ...
    let rowUsedForOpponent = row;
    let colUsedForOpponent = col;
    if(currentUpdateUI.turnIndex === 1 && shouldRotateBoard){
    ...
    let rowUsedForcheckAnimal = row;
    let colUsedForcheckAnimal = col;
    if(currentUpdateUI.turnIndex === 1 && shouldRotateBoard){
    ...
    if (!firstClicked) {
      ...
      log.info("cell clicked One done is" +cellClickedOneDone );
  ...
  export function cellClickedTwo(row: number, col: number): void {
    ...
    if (isOwn(row, col)) { // if the player select another own piece
      ...
      let rowUsedForClickOne = row;
      let colUsedForClickOne = col;
      if (currentUpdateUI.turnIndex === 1 && shouldRotateBoard) {
  ...
  export function isOpponent(row: number, col: number): boolean {
    ...
    let opponentColor = gameLogic.getTurn(1 - currentUpdateUI.turnIndex);
